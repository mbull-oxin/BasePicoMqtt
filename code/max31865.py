...
class Max31865:
    ...
    def __init__(self,bus,cs,polarity=0,rtd_nominal=100,ref_resistor=400.0,wires=2,filter_frequency=60,miso=11,mosi=12,sck=10):

        # Setup SPI
        self.cs = Pin(cs, mode=Pin.OUT)
        self.cs.low()
        self._device = SPI(bus, baudrate=200000, polarity=polarity, phase=1,
                           firstbit=SPI.MSB, sck=Pin(sck),
    ...
    def temperature(self):
        ...
        A=3.908e-3
        B=-5.775e-7
        temp=(-A + math.sqrt(A*A - 4*B*(1-raw_reading/100))) / (2*B)
        if temp>=0:
            return temp
